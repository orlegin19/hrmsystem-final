...
/*Invoice End*/   
    ...
    public function GetPaySalaryValueByID($empId)
    {
        $this->db->select('*');
        ...
        // Check if a record exists for the given emp_id
        if ($query->num_rows() > 0) {
            return $query->row();
        } else {
...
public function getPinFromID($employeeID){
      ...
      public function totalHoursWorkedByEmployeeInAMonth($employeePIN, $start_date, $end_date)
    {
	  ...
	  $sql = "SELECT sum(working_hour) AS Hours FROM `attendance` WHERE (`attendance`.`emp_id`='$employeePIN') AND (`atten_date` BETWEEN '$start_date' AND '$end_date')";
        $query  = $this->db->query($sql);
        $result = $query->result();
        return $result;
    }
    ...
    public function getOtherInfo($emid) {
      $sql = "SELECT `employee`.*,
              ...
              (SELECT SUM(working_hour) AS Hours 
			  FROM `attendance` WHERE (`attendance`.`emp_id`='$emid') AND (DATE_FORMAT(`attendance`.`atten_date`, '%m'))=MONTH(CURRENT_DATE())) AS hours_worked,COUNT(*) AS days FROM `employee`
              LEFT JOIN `department` ON `employee`.`dep_id`=`department`.`id` 
    ...
    public function DeletPayroll($id){
