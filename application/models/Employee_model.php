...
class Employee_model extends CI_Model
{
  ...
  public function GetBasic($id)
  {
    $sql = "SELECT `employee`.*,
      ...
      LEFT JOIN `attendance_schedules` ON `employee`.`schedule_id`=`attendance_schedules`.`id`
      WHERE `em_id`='$id'";
    $query = $this->db->query($sql);
    $result = $query->row();
    return $result;
  ...
  public function Update_PayDate($sid, $data)
  {
    // Assuming 'pay_salary' is your table name
    $this->db->where('pay_id', $sid);
  ...
  public function DeletEmployee($id)
  {
      ...
      // It's not necessary to execute a delete query after the update.
      // $this->db->delete('employee', array('id' => $id));
  
      return $query;
  }
  
  public function isEmIdUnique($emId, $currentId)
  {
    // Query the database to check if em_id is unique excluding the current record being updated
    $this->db->where('em_id', $emId);
    $this->db->where('id !=', $currentId);
    $query = $this->db->get('employee');

    return $query->num_rows() == 0; // If there are no matching records, em_id is unique
  }
  public function isEmCodeUnique($emCode)
  {
    $this->db->select('em_code');
    $this->db->from('employee');
    $this->db->where('em_code', $emCode);
