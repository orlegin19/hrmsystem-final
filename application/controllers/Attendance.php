...
class Attendance extends CI_Controller
{
	...
	public function DeletAttendance(){
        if($this->session->userdata('user_login_access') != False) {  
		$id= $this->input->get('D');
		$success = $this->attendance_model->DeletAttendance($id);
    ...
    public function Add_Attendance()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
				...
				$emid = $emcode->em_id;
				$sched = $this->attendance_model->getAllAttendanceScheduleByID($emid);
				//$timesched = $sched->time_in;
				if($total > 12){
					$work = '12';
				} else {
					$work = $hour->format('%h') + number_format($mins,1);
				}
