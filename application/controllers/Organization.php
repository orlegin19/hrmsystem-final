...
class Organization extends CI_Controller {
	...
	public function Branch(){
        ...
        $data['branch'] = $this->organization_model->braselect();
    ...
    public function Save_bra(){
        if($this->session->userdata('user_login_access') != False) { 
        $des = $this->input->post('branch');
    ...
    public function bra_delete($des_id){
        if($this->session->userdata('user_login_access') != False) {
        $this->organization_model->branch_delete($bra_id);
    ...
    public function Edit_bra($bra){
        if($this->session->userdata('user_login_access') != False) {
        $data['branch'] = $this->organization_model->braselect();
        $data['editbranch']=$this->organization_model->branch_edit($bra);
        $this->load->view('backend/branch', $data);
        }
    else{
		redirect(base_url() , 'refresh');
    ...
    public function Update_bra(){
        if($this->session->userdata('user_login_access') != False) {
        $id = $this->input->post('id');
        $branch = $this->input->post('branch');
        $data =  array('bra_name' => $branch );
