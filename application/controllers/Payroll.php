...
class Payroll extends CI_Controller {
   ...
   /* public function Salary_List(){
    ...
    public function GetSallaryById(){
        ...
        $data['paysalaryvalue'] = $this->payroll_model->GetPaySalaryValueByID($id);
        echo json_encode($data);
        }
    else{
        redirect(base_url() , 'refresh');
    ...
    public function DeletPayroll(){
        if($this->session->userdata('user_login_access') != False) {  
		$id= $this->input->get('D');
		$success = $this->payroll_model->DeletPayroll($id);
    ...
    public function Update_Salary1(){
        if($this->session->userdata('user_login_access') != false) {
            $sid = $this->input->post('sid'); // Salary ID
            $paydate = $this->input->post('paydate'); // New paydate
    
            // Check if the salary ID exists, and if so, update the paydate
            if (!empty($sid) && !empty($paydate)) {
                $data = array(
                    'paid_date' => $paydate
                );
    
                // Update the paydate in the pay_salary table using your model
                $success = $this->employee_model->Update_PayDate($sid, $data);
    
                if ($success) {
                    echo "Paydate Updated Successfully";
                } else {
                    echo "Failed to update paydate";
                }
            } else {
                echo "Salary ID or Paydate is missing or invalid.";
            }
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function Get_PayrollDetails(){
       ...
       foreach($holiday as $value){
        ...
        $totalmonthhour = ($monthday * 12) + 36;
        $totalmonthhour;
        $employee = $this->payroll_model->GetDepEmployee($depid);

        foreach($employee as $value){
            $hourrate = round($value->total/$totalmonthhour,2);
            echo "<tr>
                    <td>$value->em_code</td>
    ...
    public function load_employee_Invoice_by_EmId_for_pay(){
       ...
       $month_holiday_count = $this->payroll_model->getNumberOfHolidays($month, $year);
        ...
        // Hourly rate for the month
        $hourly_rate = round($employee_salary / $total_work_hours,2); //15.62
                        ...
                        <div class='card card-body'>
                            <div class='row'>
                                <div class='col-md-4 col-xs-6 col-sm-6'>
                                    <img src='$base/assets/images/jam.jpg' style=' width:180px; margin-right: 10px;' />
                                </div>
                                <div class='col-md-8 col-xs-6 col-sm-6 text-left payslip_address'>
                                    ...
                                    <p>
                                        Phone: $settingsvalue->contact
                                    </p>
                                    <p>
                                        Email: $settingsvalue->system_email
                                    </p>
                                </div>
                            </div>
                            <div class='row'>
                                ...
                                <div class='col-md-12'>
                                    <table class='table table-condensed borderless payslip_info'>
                                        ...
                                        <tr>
                                            <td>Days Worked</td>
                                            <td>: ". 
                                                ceil($salary_info->total_days / 10);
                                            echo"</td>";
                                    ...
                                    <table class='table table-condensed borderless' style='border-left: 1px solid #ececec;'>
                                        ...
                                        <tbody style='border: 1px solid #ececec;'>
                                            ...
                                            <tr>
                                                ...
                                                <td class='text-center'>$salary_info->total_days   hrs</td>
                                            </tr>
                                        ...
                                        <tfoot class='tfoot-light'>
                                            <tr>
                                                <th>Rate Per Hour</th>
                                                <th class='text-center'> ₱ ". $total_add = round(($salary_info->basic) / 360,2);  echo  "</th>
                                                <!--<th class='text-right'>".$total_did = $salary_info->basic + $salary_info->diduction;  round($total_did,2); echo "PHP</th>-->
                                            </tr>
                                            <tr>
                                                <th class='text-left'>Net Pay</th>
                                                <th class='text-center'> ₱ $salary_info->total_pay </th>
                                            </tr>
                                        </tfoot>
                                    </table>
                                </div>
